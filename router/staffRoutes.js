...
router.post("/add", upload.single("staffImage"), async (req, res) => {
  try {
    ...
    const newStaffMember = new Staff({
      ...
      contacts: {
        email: req.body.contacts?.email || "", // Set a default value if email is missing
        phoneNumber: req.body.contacts?.phoneNumber || "", // Set a default value if phoneNumber is missing
      },
      image: imagePath,
    });
